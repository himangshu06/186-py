...
def encrypt_now():
    ...
    chipher_code = encrypt('XYZ', data)
    hex_str = chipher_code.hex()
    print("Encryption : ",hex_str)
...
def decrypt_now():
    ...
    txt_file = filedialog.askopenfilename(title="open text file",filetypes=(("text files","*.txt"),))
    print(txt_file)
    name = os.path.basename(txt_file)
    txt_file = open(name,'r')
    hex_str = txt_file.read()
    bytes_str = bytes.fromhex(hex_str)
    orginal = decrypt('AIM',bytes_str)
...
def startDecryption():
    ...
    btn_open_file = Button(decryption_window, text="Choose File..",command=decrypt, font = 'arial 13')
...
def startEncryption():
    ...
    btn_create = Button(encryption_window, text="Create", font = 'arial 13',command=encrypt)
